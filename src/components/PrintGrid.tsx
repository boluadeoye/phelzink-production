...
const PrintGrid = () => {
  return (
    <section className="py-24 bg-white">
      {/* POINT 1 & 4: STRICT BOXED CONTAINER */}
      <div className="max-w-[1280px] mx-auto px-6 md:px-12">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-black text-ink mb-8 leading-[1.1] tracking-tighter font-sans">
            What we can print <br /> for you
          </h2>
          <p className="text-ink/60 max-w-2xl mx-auto text-lg leading-relaxed font-sans">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-12">
          {printItems.map((item, index) => (
            ...
            >
              <div className="relative w-full aspect-square overflow-hidden bg-[#F3F4F6] mb-4 rounded-2xl">
                <Image 
                  src={item.image}
                  alt={item.name}
                  fill
              ...
              <span className="text-sm font-bold text-ink/80 group-hover:text-emerald transition-colors font-sans">
                {item.name}
              </span>
            </motion.div>
          ))}
